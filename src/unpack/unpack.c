...
uint8_t otic_unpack_init(otic_unpack_t* oticUnpack, uint8_t(*fetcher)(uint8_t*, size_t))
{
    if (!oticUnpack)
        return 0;
    if (!fetcher){
...
uint8_t otic_unpack_defineChannel(otic_unpack_t* oticUnpack, uint8_t id, uint8_t(*flusher)(uint8_t*, size_t))
{
    ...
    oticUnpack->channels = realloc(oticUnpack->channels, sizeof(otic_unpack_channel_t*) * (oticUnpack->totalChannels + 1));
